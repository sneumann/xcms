...
- Fix title of
  ...
  plot.
- Add parameter `rtCenterFun` to
  [`PeakDensityParam()`](https://sneumann.github.io/xcms/reference/groupChromPeaks.md)
  to support specifying the function to calculate the reported retention
  time of a feature (`"rtmed"`).

### Changes in version 4.9.2

- Fix issue with
  [`refineChromPeaks()`](https://sneumann.github.io/xcms/reference/refineChromPeaks.md)
  and `MergeNeighboringPeaksParam` where in certain cases completely
  overlapping peaks were not merged (issue
  [\#825](https://github.com/sneumann/xcms/issues/825)).
...
- Add support for parameter `skipFilled` to
  [`featureSpectra()`](https://sneumann.github.io/xcms/reference/featureSpectra.md)
  for `XcmsExperiment` and `XcmsExperimentHdf5` to avoid extraction of
  spectra for gap-filled chromatographic peaks.
- Fix: add missing import of the
  [`spectraData()`](https://rdrr.io/pkg/ProtGenerics/man/protgenerics.html)
  method from *Spectra*.
- Add parameter `minMzWidthPpm` to
  ...
  allowing to define a minima *guaranteed* m/z width of the feature
  areas respectively regions to integrate data from for gap-filling
  (issue [\#756](https://github.com/sneumann/xcms/issues/756)).
...
- Fix issue in `[,XcmsExperiment`: the subset operation did not revert
  ...
  `keepAdjustedRtime = FALSE` (issue
  [\#801](https://github.com/sneumann/xcms/issues/801)).
- Fix issue in `chromPeakSpectra` for `XcmsExperiment` with duplicated
  chrom peak IDs (issue
  [\#796](https://github.com/sneumann/xcms/issues/796)).
- Fix issue in
  [`chromPeakSpectra()`](https://sneumann.github.io/xcms/reference/chromPeakSpectra.md)
  for `XcmsExperiment` that added column (sample) names to the resulting
  `XChromatograms` object with newer versions of the *MsExperiment*
  package (\>= 1.11.1); issue
  [\#803](https://github.com/sneumann/xcms/issues/803).

### Changes in version 4.7.1

- Change the naming convention of chromatographic peaks to include also
  the MS level after `"CP"`: `"CP1001"` instead of `"CP001"` for a
...
- Optimization and performance improvements for extraction of
  chromatographic data. This includes using
...
- Throw an error if parameters `mz` or `rt` for
  [`chromatogram()`](https://sneumann.github.io/xcms/reference/chromatogram-method.md)
...
- Fix a bug in
  ...
  that caused an error when only a single chrom peak was added.

### Changes in version 4.5.3

- Address issue [\#765](https://github.com/sneumann/xcms/issues/765):
  peak detection on chromatographic data: report a chromatogram’s
  `"mz"`, `"mzmin"` and `"mzmax"` as the mean m/z and lower and upper
  m/z in the
...
- Add `chromPeakSummary` generic (issue
  [\#705](https://github.com/sneumann/xcms/issues/705)).
- Add
  [`chromPeakSummary()`](https://sneumann.github.io/xcms/reference/chromPeakSummary.md)
  method to calculate the *beta* quality metrics.
- Add [`c()`](https://rdrr.io/r/base/c.html) method to combine multiple
  `XcmsExperiment` objects into one.
...
- Small update to
  ...
  [`chromPeaks()`](https://sneumann.github.io/xcms/reference/XCMSnExp-class.md)
  and `featuresDefinitions()` columns to be added to the `Spectra`
  output.
...
- Small update to the `matchLamaChromPeaks()` function to get the
  chromPeaksId of the chromPeaks matched with Lamas.
- Small fix to the .yml file for the github actions, so they do not
  crash on warnings.

### Changes in version 4.3.3

- Fix issue [\#755](https://github.com/sneumann/xcms/issues/755):
  [`chromatogram()`](https://sneumann.github.io/xcms/reference/chromatogram-method.md)
  with `msLevel = 2` fails to extract chromatographic data if
  `isolationWindowTargetMz` is not specified or available (e.g. for MSe
  data).
...
- Change
  ...
  to a method and add an implementation for `MsExperiment` objects.
...
- Support excluding samples or sample groups from defining features with
  *PeakDensity* correspondence analysis (issue
  [\#742](https://github.com/sneumann/xcms/issues/742)).
- Add
  [`plotPrecursorIons()`](https://sneumann.github.io/xcms/reference/plotPrecursorIons.md)
  function.
- Fix in
  [`dropFeatureDefinitions()`](https://sneumann.github.io/xcms/reference/XCMSnExp-class.md)
  that was not correctly removing additional metadata from gap-filled
...
- Fix for issue [\#734](https://github.com/sneumann/xcms/issues/734).
  XIC plot is is now working with MS2 Data.
...
- Implementation of the `LamaParama` class and method for the
  ...
  function. Allowing alignment of a dataset based on landmarks (lamas)
  from an external reference dataset.
- Implementation of related user-level function
  [`matchLamasChromPeaks()`](https://sneumann.github.io/xcms/reference/LamaParama.md),
  `summarizeMatchLama()` and `plot(LamaParama)` which allows for
  evaluation of matching between lamas and chromPeaks.
...
- Fixing issue [\#716](https://github.com/sneumann/xcms/issues/716):
  edit of `.empty_chrom_peaks` function so an `sn` column is returned.
  Fixes extracting and plotting of peaks after using `manualChromPeaks`

### Changes in version 4.1.7

- Implementation of `filterFeatures` function with `filter` parameters:
  `RsdFilter`, `DratioFilter`, `PercentMissingFilter`, `BlankFlag`. They
  can be used ot filter features from `XcmsResult` and
  `SummarizedExperiment` objects.
- Addition of a section in the main xcms vignette to describe how to use
  it.
...
- Fix definition of anchor peaks for peakGroups alignment with subset
  (issue [\#702](https://github.com/sneumann/xcms/issues/702)).
- Add `filterMsLevel` method for `MsExperiment` and `XcmsExperiment`.
- Ensure chunk-wise processing of Spectra (introduced with version
  1.13.2) is disabled when xcms is using its own chunk-wise processing.

### Changes in version 4.1.3

- Add parameter `verboseBetaColumns` to `CentWaveParam` to enable
  calculation of additional peak quality metrics comparing the EIC to an
  idealized bell curve.

### Changes in version 4.1.2

- Add a `param =` to generic function `storeResults`: `PlainTextParam`
  to save an `XcmsExperiment` or `MsExperiment` object as colleciton of
  plain text files.

### Changes in version 4.1.1

- Add method `storeResults` and one of its `param =`: `RDataParam` to
  save an `XcmsExperiment` object as an .RData file.

## xcms 3.99

### Changes in version 3.99.6

- Add method to coerce a `XcmsExperiment` to a `xcmsSet` (issue
  [\#696](https://github.com/sneumann/xcms/issues/696)).
...
- Fix issue in `refineChromPeaks` with `MergeNeighboringPeaksParam`
  where a wrong apex position was considered in the evaluation whether
...
- Re-write the `reconstructChromPeakSpectra` for DIA data analysis to
  fix an issue with chromatographic peaks in overlapping SWATH isolation
  windows and generally to improve performance.
...
- Fix issue in `chromatogram` after filtering a result object (issue
  [\#511](https://github.com/sneumann/xcms/issues/511)).
...
- Fix error with `fillChromPeaks` on sparse data (many empty spectra)
  and peak detection performed with `MatchedFilterParam` (issue
  [\#653](https://github.com/sneumann/xcms/issues/653)).
- Update to newer function names in the `rgl` package (issue
  [\#654](https://github.com/sneumann/xcms/issues/654)).
...
- Change the way the m/z value for a chromatographic peak is determined
  by centWave: if a ROI contains more than one peak for one scan
  (spectrum) an intensity-weighted m/z is reported for that scan. The
  m/z of the chromatographic peak is then calculated based on these
  reported m/z values for each scan (spectrum). In the original version
  the mean m/z for a scan was reported instead. As a result, m/z values
  of chromatographic peaks are now slightly different but are expected
  to be more accurate. See <https://github.com/sneumann/xcms/issues/590>
  for more details.
...
- Use mzML instead of mzData files in testing and vignettes, since mzR
  drop mzData reading and msdata package will drop mzData files as well
...
- Fix issue [\#545](https://github.com/sneumann/xcms/issues/545): skip
  second centWave run with CentWavePredIsoParam in regions of interest
  with undefined peak boundaries/scan ranges.
...
- Add `filterChromPeaks` method for `XCMSnExp` (issue
  [\#541](https://github.com/sneumann/xcms/issues/541)).
...
- Import `filterIntensity`, `normalize` and `alignRt` for `Chromatogram`
  and `MChromatograms` from `MSnbase`.
...
- `align,Chromatogram` gains new method `"none"` which will only keep
  ...
  (much faster) matching function `closest` from the `MsCoreUtils`
  package is used.
- Method `correlate,Chromatogram` gains parameter `useIntensitiesAbove`
  to perform the correlation only with values larger than this threshold
  (avoiding thus high correlation because of many 0-values).
- Add method `filterIntensity,Chromatogram` that allows to filter a
  chromatogram object keeping only data points with an intensity above a
...
- Add `FilterIntensityParam` to filter chromatographic peaks on
  intensity (issue
  [\#502](https://github.com/sneumann/xcms/issues/502)).
...
- Change from `Spectra` and `Chromatograms` to `MSpectra` and
  `MChromatograms` from MSnbase version \>= 2.15.3.
...
- `reconstructChromPeakSpectra`: ensure a retention time is reported for
  reconstructed MS2 spectra (issue
  [\#485](https://github.com/sneumann/xcms/issues/485)).
...
- Potential fix for issue
  [\#481](https://github.com/sneumann/xcms/issues/481): function should
  no longer throw an error because retention times are of length 0.
- More efficient splitting of processing which should increase the speed
  of the findChromPeaks, refineChromPeaks, reconstructChromPeakSpectra
  and chromPeakSpectra calls.

### Changes in version 3.11.1

- Fix issue [\#471](https://github.com/sneumann/xcms/issues/471):
  conversion from `XCMSnExp` to `xcmsSet` looses phenodata (thanks to
  Andris Jankevics for reporting and providing a solution).
...
- `featureChromatograms` gets new parameter `n` and `value` to extract
  EICs only from the top n samples with highest intensities.
...
- Fix issue in centWave which skips peak detection depending on minimum
  peakwidth (issue
  [\#445](https://github.com/sneumann/xcms/issues/445)): add parameter
  `extendLengthMSW` in `CentWaveParam`. Thanks to William Kumler for
  contributing the fix.
- Tentatively reduce memory requirements in `fillChromPeaks`.
- Fix issue [\#467](https://github.com/sneumann/xcms/issues/467) for
  fillPeaks() of an xcmsSet converted from an XCMSnSet

### Changes in version 3.9.3

- Move multtest from Imports to Suggests to avoid duplicated method
  definition for plot (issue
  [\#459](https://github.com/sneumann/xcms/issues/459)).
...
- Fine-tune MergeNeighboringPeaks peak refinement method: the average of
  the 3 data points between candidate peaks is used to evaluate whether
  the peaks should be merged making the approach more robust against
  outliers. In addition, an ion chromatogram for candidate peaks is
  extracted with an m/z range expanded depending on the expandMz and ppm
  setting ensuring that low intensity data points between candidate
  peaks are not missed out (because their m/z might be slightly shifted
  on ToF instruments). The mzmin and mzmax of the merged peak represents
  also the minimum and maximum m/z of all data points in that extracted
  ion chromatogram.
...
- Add refineChromPeaks method and CleanPeaksParam class to allow removal
  of chromatographic peaks exceeding a user-definable maximal peak
  width.
- Add MergeNeighboringPeaksParam for refineChromPeaks to allow merging
  of chromatographic peaks close in m/z and retention time with a signal
  between them higher than a certain threshold (issue
  [\#414](https://github.com/sneumann/xcms/issues/414)).
...
- mzClust correspondence analysis: check and fix missing values in
  column mz of the peaks matrix (issue
  [\#416](https://github.com/sneumann/xcms/issues/416)).
...
- Fix documentation (issue
  [\#401](https://github.com/sneumann/xcms/issues/401)).
...
- Fix problem in featureChromatograms with include = “feature_only” that
  could return a non-valid object.
...
- Fix issue [\#359](https://github.com/sneumann/xcms/issues/359).
- Fix issue [\#360](https://github.com/sneumann/xcms/issues/360): rawEIC
  skipped last scan/spectrum if rtrange was provided.
- filterMsLevel keeps now chromatographic peaks and feature definitions
  from the specified MS levels (issue
  [\#362](https://github.com/sneumann/xcms/issues/362)).
- Fix bug in `xcmsRaw` that leads to a netCDF error message (issue
  [\#363](https://github.com/sneumann/xcms/issues/363)).
...
- Fix issue [\#349](https://github.com/sneumann/xcms/issues/349).
- Add updateObject function for XCMSnExp objects (issue
  [\#347](https://github.com/sneumann/xcms/issues/347)).
...
- Fix matchedFilter peak detection problems with empty spectra (issue
  [\#325](https://github.com/sneumann/xcms/issues/325)).
...
- Add featureValues method for XChromatograms objects (issue
  [\#336](https://github.com/sneumann/xcms/issues/336)).
- Add correspondence analysis (peak grouping) for chromatographic data
  (for now only with PeakDensity method; issue
  [\#336](https://github.com/sneumann/xcms/issues/336)).
...
- Alignment based on a sample subset for the peak groups method (issue
  [\#335](https://github.com/sneumann/xcms/issues/335)): sample subset
  can be defined with the subset parameter, samples not included in the
  subset will be aligned based on the adjusted retention times of the
  closest sample in the subset.
- Add findChromPeaks,XChromatograms (issue
  [\#332](https://github.com/sneumann/xcms/issues/332)).
- Add processHistory,XChromatograms.
- Add plot,XChromatograms method with automatic peak highlighting (issue
  [\#334](https://github.com/sneumann/xcms/issues/334)).
...
- findChromPeaks,Chromatogram returns an XChromatogram object (issue
  [\#329](https://github.com/sneumann/xcms/issues/329)).
- Add chromPeaks,XChromatogram (issue
  [\#329](https://github.com/sneumann/xcms/issues/329)).
- Add XChromatogram object (issue
  [\#329](https://github.com/sneumann/xcms/issues/329)).
...
- Add example on clustering of base peak chromatograms to the vignette
  (issue [\#328](https://github.com/sneumann/xcms/issues/328)).
- Small fix in the vignette (issue
  [\#327](https://github.com/sneumann/xcms/issues/327)).
- Add parameter groupval to exportMetaboAnalyst (issue
  [\#296](https://github.com/sneumann/xcms/issues/296)).
...
- Add chromPeakSpectra and featureSpectra functions to extract MS2
  ...
  [\#321](https://github.com/sneumann/xcms/issues/321)).
- Fix profMat to handle also data files with empty spectra (issue
  [\#312](https://github.com/sneumann/xcms/issues/312)).
- Add argument ylim to plotAdjustedRtime (issue
  [\#314](https://github.com/sneumann/xcms/issues/314)).
- Add imputeRowMin and imputeRowMinRand, two simple missing value
  imputation helper functions.
- Fix additional problem mentioned in issue
  [\#301](https://github.com/sneumann/xcms/issues/301) with obiwarp
  retention time correction if some spectra have m/z values of `NA`.
- Fix issue [\#300](https://github.com/sneumann/xcms/issues/300)
  avoiding chromatographic peaks with rtmin \> rtmax.
- Fixes for issues [\#291](https://github.com/sneumann/xcms/issues/291),
  [\#296](https://github.com/sneumann/xcms/issues/296).
...
- The chromPeaks matrix has now rownames to uniquely identify
  chromatographic peaks in an experiment. Chromatographic peak IDs start
  with “CP” followed by a number.

### Changes in version 3.3.2

- Add writeMSData method for XCMSnExp allowing to write mzML/mzXML files
  with adjusted retention times (issue
  [\#294](https://github.com/sneumann/xcms/issues/294)).
- Fix profEIC call for single-scan-peak (pull request
  [\#287](https://github.com/sneumann/xcms/issues/287) from
  [@trljcl](https://github.com/trljcl)).
- Fix centWave avoiding that the same peak is reported multiple times if
  fitgauss = TRUE is used (issue
  [\#284](https://github.com/sneumann/xcms/issues/284)).
- featureSummary reports also RSD (relative standard deviations) of
  features across samples (issue
  [\#286](https://github.com/sneumann/xcms/issues/286)).
...
- Fix a bug in fillChromPeaks that would return the integrated signal
  being Inf.
- Fix for issue [\#267](https://github.com/sneumann/xcms/issues/267):
  error in fillChromPeaks when the retention time of the peaks are
  outside of the retention time range of certain files.
...
- Parameter ‘type’ added to plotChromPeakDensity and ‘whichPeaks’ to
  ...
  of chromPeaks.
- Parameter ‘type’ in chromPeaks gets additional option “apex_within” to
  return chromatographic peaks that have their apex within the defined
  rt and/or m/z range.
- Add functions rla and rowRla to calculate RLA (relative log
  abundances).
- Add peaksWithMatchedFilter to perform peak detection in
  chromatographic (MRM/SRM) data (issues
  [\#277](https://github.com/sneumann/xcms/issues/277) and
  [\#278](https://github.com/sneumann/xcms/issues/278)).
- Add peaksWithCentWave to perform centWave peak detection in
  chromatographic (MRM/SRM) data (issue
  [\#279](https://github.com/sneumann/xcms/issues/279)).
- Add findChromPeaks,Chromatogram methods for CentWaveParam and
  MatchedFilterParam (issue
  [\#280](https://github.com/sneumann/xcms/issues/280)).
...
- Fix misplaced parenthesis in the check for multiple spectra in
  findChromPeaks,OnDiskMSnExp,MSWParam. Thanks to
  [@RonanDaly](https://github.com/RonanDaly) (PR
  [\#276](https://github.com/sneumann/xcms/issues/276)).
- Update link to correct metlin page in diffreport result (issue
  [\#204](https://github.com/sneumann/xcms/issues/204)).

### Changes in version 3.1.2

- Add filterFeatureDefinitions function.
- Fix [\#273](https://github.com/sneumann/xcms/issues/273): better error
  message in case not a single feature could be defined by
  groupChromPeaks.

### Changes in version 3.1.1

- Reading raw files using xcmsSet or xcmsRaw uses now the automatic file
  type detection feature from mzR.
- c function to concatenate XCMSnExp objects.
- groupnames method for XCMSnExp objects (issue
  [\#250](https://github.com/sneumann/xcms/issues/250)).
- Fix [\#237](https://github.com/sneumann/xcms/issues/237):
  findPeaks.MSW was not throwing an error if applied to multi-spectrum
  MS file.
- Fix [\#249](https://github.com/sneumann/xcms/issues/249): quantile
  call in adjustRtime PeakGroups without na.rm = TRUE.
- Fix [\#259](https://github.com/sneumann/xcms/issues/259)

## xcms 2.99

### Changes in version 2.99.10

- Fix [\#230](https://github.com/sneumann/xcms/issues/230): Failing
  vignettes on Windows.

### Changes in version 2.99.9

- Chromatographic peak detection uses adjusted retention times on an
  aligned XCMSnExp object (issue
  [\#213](https://github.com/sneumann/xcms/issues/213),
  [\#208](https://github.com/sneumann/xcms/issues/208)).
...
- Add parameter msLevel to chromatogram,XCMSnExp method (issue
  [\#205](https://github.com/sneumann/xcms/issues/205)).
- Obiwarp alignment is now performed on one MS level and adjustment is
  applied to all MS levels (issue
  [\#214](https://github.com/sneumann/xcms/issues/214)).
...
- Add argument msLevel = 1L to extractMsData method (issue
  [\#223](https://github.com/sneumann/xcms/issues/223)).
- New applyAdjustedRtime function to consolidate the alignment results,
  i.e. replace the raw retention times in the XCMSnExp with the adjusted
  retention times.
...
- Argument ‘sampleGroups’ is mandatory for ‘PeakDensityParam’ (issue
  [\#228](https://github.com/sneumann/xcms/issues/228)).
- Fix [\#191](https://github.com/sneumann/xcms/issues/191): Excessive
  memory use in fillPeaks.
- Fix [\#220](https://github.com/sneumann/xcms/issues/220): peaks matrix
  is missing column “sample” if no peaks were found in the first sample.
- Fix [\#222](https://github.com/sneumann/xcms/issues/222):
  findChromPeaks does not return an XCMSnExp object filtered to a single
  MS level despite peak detection is performed on a single level.
- Fix problem in plotMsData causing wrong colors to be used to label the
  data points.
...
- Fix [\#201](https://github.com/sneumann/xcms/issues/201): Warnings:
  ‘readMSData2’ is deprecated, thanks to L. Gatto.
...
- Replace xcmsDirect.Rnw with rmarkdown-based vignette using the new
  user interface.
- issue [\#196](https://github.com/sneumann/xcms/issues/196): removed
...
- Polarity information was not read from mzXML files (issue
  [\#192](https://github.com/sneumann/xcms/issues/192)).

### Changes in version 2.99.3

- issue [\#188](https://github.com/sneumann/xcms/issues/188): determine
  file type from file content if file ending not known.

### Changes in version 2.99.2

- issue [\#181](https://github.com/sneumann/xcms/issues/181): problem
  when isCentroided,Spectrum method returns NA because of too few peaks
  in a spectrum. Fixed by checking in such cases all spectra in the
  file.
- issue [\#184](https://github.com/sneumann/xcms/issues/184): add
  parameter sleep to do_groupChromPeaks_density function to be backwards
  compatible with the old group.density code.

### Changes in version 2.99.1

- extractMsData to extract raw MS data as a data.frame (issue
  [\#120](https://github.com/sneumann/xcms/issues/120)).
- issue [\#175](https://github.com/sneumann/xcms/issues/175): an error
  is now thrown if no peak group was identified for peak group retention
  time correction.
- issue [\#178](https://github.com/sneumann/xcms/issues/178): scanrange
  was collapsed when the adjusted range was reported (pull request by
  Jan Stanstrup).
- issue [\#180](https://github.com/sneumann/xcms/issues/180): error when
  both parameters method and smooth are provided in the retcor method.
...
- New parameter missing for extractChromatograms allowing to specify the
  intensity value to be used for rts for which no signal is available
  within the mz range.
...
- Increase parameter n for the density call in the peak density
  ...
  using small n (issue
  [\#161](https://github.com/sneumann/xcms/issues/161)). Thanks to Jan
  Stanstrup.
...
- Parameter “filled” for featureValues (issue
  [\#157](https://github.com/sneumann/xcms/issues/157)).
- Parameters “rt” and “mz” in chromPeaks method allowing to extract
  chromatographic peaks from the specified ranges (issue
  [\#156](https://github.com/sneumann/xcms/issues/156)).
- Fixed possible memory problem in obiwarp (issue
  [\#159](https://github.com/sneumann/xcms/issues/159)).
- Update getPeaks to use non-deprecated API (issue
  [\#163](https://github.com/sneumann/xcms/issues/163)).

### Changes in version 1.51.10

- filterRt for Chromatogram class (issue
  [\#142](https://github.com/sneumann/xcms/issues/142)).
- adjustRtimePeakGroups function (issue
  [\#147](https://github.com/sneumann/xcms/issues/147)).
- adjustRtime,XCMSnExp,PeakGroupsParam and do_adjustRtime_peakGroups
  support use of pre-defined matrix to perform alignment (issue
  [\#153](https://github.com/sneumann/xcms/issues/153)).
- plotAdjustedRtime to visualize alignment results (issue
  [\#141](https://github.com/sneumann/xcms/issues/141)).
- featureDefinitions and featureValues return DataFrame and matrix with
  rownames corresponding to arbitrary feature IDs (issue
  [\#148](https://github.com/sneumann/xcms/issues/148)).
- New peakGroupsMatrix slot for PeakGroupsParam class (issue
  [\#153](https://github.com/sneumann/xcms/issues/153)).
- Issue [\#146](https://github.com/sneumann/xcms/issues/146): ensure
  adjusted retention times returned by the peakGroups method to be in
  the same order than the raw retention times.
...
- Issue [\#135](https://github.com/sneumann/xcms/issues/135): fix peak
  signal integration for centWave.
- Issue [\#139](https://github.com/sneumann/xcms/issues/139): problem
  with expand.mz and expand.rt in fillPeaks.chrom.
- Issue [\#137](https://github.com/sneumann/xcms/issues/137): Error in
  findChromPeaks if no peaks are found.

### Changes in version 1.51.8

- Add Chromatogram class and extractChromatograms method.
- Issue [\#118](https://github.com/sneumann/xcms/issues/118): failing
  unit test on Windows build machine.
- Issue [\#133](https://github.com/sneumann/xcms/issues/133): error with
  c() and xcmsSet without peaks.
- Issue [\#134](https://github.com/sneumann/xcms/issues/134): xcmsSet
  constructor endless loop.
...
- Issue [\#127](https://github.com/sneumann/xcms/issues/127): failing
  unit test on Windows build machine.
...
- Issue [\#124](https://github.com/sneumann/xcms/issues/124)
  (filterRt,XCMSnExp returned empty object).
...
- mz, intensity and rtime methods for XCMSnExp allowing to return values
  grouped by sample.
- Issue [\#99](https://github.com/sneumann/xcms/issues/99) (rtrange
  outside of retention time range in getEIC,xcmsSet).
- Issue [\#101](https://github.com/sneumann/xcms/issues/101) (xcmsRaw
  function returns NULL if mslevel = 1 is specified).
- Issue [\#102](https://github.com/sneumann/xcms/issues/102) (centWave
  returns empty matrix if scales not OK). Thanks to J. Stanstrup.
- Issue [\#91](https://github.com/sneumann/xcms/issues/91) (warning
  instead of error if no peaks in ROI). Thanks to J. Stanstrup.

### Changes in version 1.51.4

- added deepCopy to avoid corrupting the original object, thanks to J.
  Stanstrup, closes [\#93](https://github.com/sneumann/xcms/issues/93)
...
- Add new CentWaveParam, MatchedFilterParam, MassifquantParam, MSWParam
  and CentWavePredIsoParam parameter class to perform method dispatch in
  the detectFeatures method.
...
- Fixed bug in rawMat (issue
  [\#58](https://github.com/sneumann/xcms/issues/58)).
- Fix issue [\#60](https://github.com/sneumann/xcms/issues/60):
  findPeaks.massifquant always returns a xcmsPeaks object.

### Changes in version 1.51.2

- As suggested by Jan Stanstrup, do not error if a centWave ROI contains
  no data, closes [\#90](https://github.com/sneumann/xcms/issues/90)

### Changes in version 1.51.1

- Fix incorrrect indexing getEIC function reported by Will Edmands,
  closes [\#92](https://github.com/sneumann/xcms/issues/92)
...
- Peak Picking function findPeaks.centWaveWithPredictedIsotopeROIs() and
  findPeaks.addPredictedIsotopeFeatures(), which allow more sensitive
  detection of isotope features.
...
- Fix getXcmsRaw that would prevent retention time correction to be
  applied (issue [\#44](https://github.com/sneumann/xcms/issues/44)
  reported by Aleksandr).
...
- Fix problem in getEIC on xcmsSet objects reported by Alan Smith in
  issue [\#7](https://github.com/sneumann/xcms/issues/7) and add a RUnit
  test case to test for this (test.issue7 in runit.getEIC.R).
...
- new slots for xcmsSet:
  ...
  - scanrange: to keep track of the scanrange argument of the xcmsSet
    method.
- new methods for xcmsRaw:
  - levelplot: similar to the image method, plots m/z vs RT with color
    coded intensities.
...
- new methods for xcmsSet:
  - getXcmsRaw: returns a xcmsRaw object for one or more files in the
    xcmsSet, eventually applying retention time correction etc.
  - levelplot: similar to the image method, plots m/z vs RT with color
    coded intensities. Allows in addition to highlight identified peaks.
  - mslevel: returns the value for the mslevel slot. For downstream
    compatibility, this method returns NULL if the object does not have
    the same named slot.
  ...
  - scanrange: returns the value for the scanrange slot. For downstream
    compatibility, this method returns NULL if the object does not have
    the same named slot.
- show method for xcmsSet updated to display also informations about the
  mslevel and scanrange.
- Elaborated some documentation entries.
- rtrange and mzrange for xcmsRaw method plotEIC use by default the full
  RT and m/z range.
- Added arguments “lty” and “add” to plotEIC method for xcmsRaw.
- getEIC without specifying mzrange returns the ion chromatogram for the
  full m/z range (i.e. the base peak chromatogram).
- Checking if phenoData is a data.frame or AnnotatedDataFrame and throw
  an error otherwise.
- xcmsSet getEIC method for water Lock mass corrected files for a subset
  of files did not evaluate whether the specified files were corrected.
...
- Major memory reduction in parallel fillPeaks() thanks to Jan
  Stanstrup. Now using an environment to mirror gvals to each list item
  in the very large argList.
...
- fixed indexing bug in group.nearest, which under certain circumstances
  caused all peaks in the first sample to be ignored (reported by Tony
  Larson)
...
- Behind the scenes xcms now uses the xcmsSource class to read raw data.
  This allows e.g. to write a class that pulls raw data from e.g. a
  database
...
- diffreport and plotEIC have a new parameter mzdec, with is the number
  of decimal places of the m/z values in the EIC plot title

### Changes in version 1.33.16

- Lock mass gap filler now works with netCDF lock mass function file to
  find the exact times of the scans and works with the newer Waters MS
  instruments.
...
Other Changes since Version 1.32: - improved mzData writing, now
includes MSn spectra and less verbose. - improved netCDF writing, but
not yet good enough for AMDIS

### CHANGED BEHAVIOUR since Version 1.14:

- centWave may report a smaller set of peaks, due to a small bug in the
  ROI algorithm some features with mass deviation \> ppm were retained.
...
- New method for grouping: an algorithm inspired by mzMine
  ...
  group(method=“density”). It can individually group close-eluting peaks
  of very similar mass

- New method for retention time correction: The retcor(method=“obiwarp”)
  algorithm operates on the raw data, and thus allows to correct runs
  without well-behaving peak groups, or without peak picking at all.

- fillPeaks(method=“MSW”) is now also available for direct infusion
  spectra. The findPeaks(method=“MSW”) now returns several intensities,
  and correctly reports mzmin and mzmax for peaks.
...
- centWave gains an optional argument “noise”, which is useful for data
  that was centroided without any intensity threshold, centroids with
  intensity \< “noise” are omitted from ROI detection

- the fillPeaks() methods now remember which was an observed, and which
  was a “filled” peak.
