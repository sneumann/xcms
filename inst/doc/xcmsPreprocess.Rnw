...
format NetCDF files as well as mzXML files. All data to be analyzed
by \Rpackage{xcms} must first be converted to either of those file
...
After exporting all files to NetCDF/mzXML format, they should be put in
...
for NetCDF/mzXML files. Alternatively, you can manually specify the
