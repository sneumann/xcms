...
approach.  The algorithm is useful only for centroid LC/MS data. If
your machine software or converter does not support the export of peak
centroids, tools like the \emph{TOPP-PeakPicker} can be used for
preprocessing \footnote{\url{http://www.openms.de}}.  Due to the fact
that peak centroids are used, a binning step is not necessary.

In the first phase of the algorithm areas of high peak density
(characterised by having at least \Rfunarg{minEntries} peaks within
\Rfunarg{dev} ppm in consecutive scans) in the LC/MS map are located.
In the second phase these \emph{regions of interest} (ROI) are further
analysed.  Gaussian curves are fitted to the chromatographic peaks,
and areas larger than 2*\Rfunarg{minPeakWidth} scans are checked
whether they contain more than one peak (these may also overlap) using
wavelet analysis. 

This algorithm is much slower than \Rmethod{findPeaks.matchedFilter}
due to the extensive gaussian fits.  It can be sped up by
narrowing the ROI's found in the first phase. This can be done by
restricting the parameter \Rfunarg{dev} (default value is 140~ppm for
a 10~ppm QTOF), depending on your machine's mass accuracy.
