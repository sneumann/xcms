...
project.  (\url{http://www.bioconductor.org/}) The second is the
web site of the METLIN Metabolite Database.
...
\Rpackage{genefilter}, \Rpackage{reposTools}, and \Rpackage{multtest}
with the \texttt{Packages > Install package(s)...} menu item.
...
\item Several of the functions in \Rpackage{xcms} require an X11
display device. If it is not already installed, you will need to
install
X11\footnote{\url{http://www.apple.com/downloads/macosx/apple/x11formacosx.html}}.
Additionally, because Bioconductor is currently between releases,
you will need to install the Apple Developer
Tools\footnote{\url{http://connect.apple.com/}} to allow installation
of several of the Bioconductor packages.

\item Download and install the current version of \texttt{R}.
Detailed instructions for doing so are available on the CRAN site.

\item From the Developmental Packages section of the Bioconductor
web site, download the \Rpackage{Biobase} and \Rpackage{multtest}
packages. Also, download the Mac OS X binaries of the \Rpackage{xcms}
and \Rpackage{faahKO} packages.

\item Launch \texttt{R} and open the R Package Installer using the
\texttt{Packages \& Data > Package Installer} menu item. Select
``Local Source Package'' from the popup menu and the ``At User
Level'' radio button.

\item Now install the four packages you downloaded in the previous
step.
...
listing as an official dependency. It is currently used by a single
