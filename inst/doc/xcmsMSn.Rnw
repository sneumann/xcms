...
<<RawFiles>>=
mzdatapath <- system.file("mzdata", package = "msdata")
...
A xcmsFragments-Object contains a Table with informations about all Peaks from MS1 to MSn of one Experiment, including mz-value, retensiontime, MSlevel, and parentpeak. The data can be used for visualization or (later) computing alignments between different Experiments. The object is created as empty and can be filled with collect(). This method requires a peaktable or a xcmsSet for the MS1peaks and the original xcmsRaw for the msn-Data.

<<SpectraSelection>>=
peaks <- findPeaks(xraw, method="MS1")
frag <- xcms:::xcmsFragments()
xfrag <- collect(frag, peaks@.Data, xraw)
xfrag
@

plotTree prints out a simplyfied tree of the Peaks in the Object. The user can define borders for the retensionTime and for the MassRange, he can also print the tree for a single MS1-Peak.
<<SpectraSelection>>=
plotTree(xfrag,xcmsFragmentPeakID=3)
@


Finally, to visualise the fragmentation tree, all specified scans
