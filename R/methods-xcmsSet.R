...
setMethod("group.density", "xcmsSet", function(object, bw = 30, minfrac = 0.5, minsamp = 1,
    ...
    for (i in seq(length = length(mass)-2)) {
        ...
        den <- density(speakmat[,"rt"], bw, from = retrange[1]-3*bw, to = retrange[2]+3*bw, n=max(512, 2^(ceiling(log2(diff(retrange)/bw)))))
        maxden <- max(den$y)
        deny <- den$y
...
setMethod("plotrt", "xcmsSet", function(object, col = NULL, ty = NULL, leg = TRUE, densplit = FALSE) {
    ...
    if (densplit) {
        ...
        screen(2)
        par(mar = c(5.1, 4.1, 0, 2), yaxt = "n")
        allden <- density(object@peaks[,"rt"], bw = diff(rtrange)/200, from = rtrange[1], to = rtrange[2], n=max(512, 2^(ceiling(log2(diff(rtrange)/bw)))))[c("x","y")]
