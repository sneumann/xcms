...
#'     m/z deviation in consecutive scans in the LC/MS map. These ROIs are then
#'     subsequently analyzed using continuous wavelet transform (CWT) to locate
#'     chromatographic peaks on different scales. The first analysis step is
#'     skipped, if regions of interest are passed with the \code{roiList}
#'     parameter.
#'
...
peaksWithCentWave <- function(int, rt,
    ...
    ## Avoid NAs
    int[is.na(int)] <- 0
    ## Need to identify potential peaks (aka ROI)
    ## use the method for MS peak detection?
    ## use something like: identify peaks that are larger than snthreshold?
    ## Identify the "peaks" and use the peakwidth parameter to define the ROI.
...
#'
#' @return `matrix` with two columns `"scmin"` and `"scmax"` with the index of
#'     (lower and upper) defining the region of interest.
#' 
#' @md
#'
#' @noRd
...
.getRtROI <- function(int, rt, peakwidth = c(20, 50), noise = 0, prefilter = c(3, 100)) {
    ...
    ok <- vapply(roi_idxs,
                 ...
                 USE.NAMES = FALSE)
    if (any(ok))
        cbind(scmin = scmin[ok], scmax = scmax[ok])
    else
