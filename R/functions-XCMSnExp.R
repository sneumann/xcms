...
featureChromatograms <- function(x, expandRt = 0, aggregationFun = "max",
    ...
    if (include == "feature_only") {
        ...
        ## Additional loop to allow duplicated features in a row.
        ## fts <- lapply(seq_len(nr), function(r) {
        ##     pks_row <- pks_sub[pks_sub[, "row"] == r, , drop = FALSE]
        ##     if (nrow(pks_row)) {
        ##         tmp <- .subset_features_on_chrom_peaks(
        ##             ft_defs[[r]], pks_all, pks_row)
        ##         tmp$row <- r
        ##         tmp
        ##     }
        ##     else NULL
