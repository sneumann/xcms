...
#'     [Spectra] object or a simple `list`. See below for more
#'     information.
#' 
#' @return
#'
#' What object is returned depends on the value of `return.type`:
#' 
...
chromPeakSpectra <- function(x, msLevel = 2L, expandRt = 0, expandMz = 0,
    ...
    if (return.type == "Spectra") {
        ...
        if (length(res)) {
            res <- Spectra(unlist(res),
                           elementMetadata = DataFrame(peak_id = pids))
        } else res <- Spectra()
    }
    res
}
    ...
    if (return.type == "Spectra") {
        ...
        res <- Spectra(
            ...
            elementMetadata = DataFrame(
                feature_id = fids,
                peak_id = vapply(strsplit(names(res), ".", TRUE),
                                 `[`, character(1), 2)))
    } else res <- Spectra()
    res
}
