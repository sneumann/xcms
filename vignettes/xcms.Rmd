...
These criteria are typically derived from statistical formulas applied to full
rows of data, where each row represents a feature. The `filterFeatures`
function provides a robust solution for filtering features based on these
conventional quality assessment criteria. It supports multiple types of
filtering, allowing users to tailor the filtering process to their specific
needs, all controlled by the `filter` argument. This function and its
...
assessment and filtering on both the faahko and res objects. The `filter`
argument can accommodate various types of input, each determining the
specific type of quality assessment and filtering to be performed.

The `RsdFilter` enable users to filter features based on their coefficient of
variation for a specified `threshold`. It is recommended to base the
