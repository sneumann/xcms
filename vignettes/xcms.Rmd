...
- `subsetAdjust = "previous"`: adjust the retention times of a non-subset
  ...
  *B4* with *A* representing QC samples and *B* study samples, using
  `subset = c(1, 4, 7)` and `subsetAdjust = "previous"` would result in all *A*
  samples to be aligned with each other and non-subset samples *B1*, *B2* being
  adjusted based on the alignment result of subset samples *A1* and *B3*, *B4*
  on those of *A2*.
...
- `subsetAdjust = "average"`: adjust retention times of non-subset samples based
  ...
  *A2*. Since there is no subset sample after non-subset samples *B2* and *B4*
