...
```{r filled-not-filled, eval = FALSE}
...
```{r feature-rt-mz-plot, fig.width = 8, fig.height = 6, fig.cap = "Plot of retention times and m/z for all features in the data set.", eval = FALSE}
...
```{r feature-groups-rtime-plot, fig.width = 8, fig.height = 6, fig.cap = "Feature groups defined with a rt window of 10 seconds", eval = FALSE}
...
```{r feature-groups-rtime-plot2, fig.width = 8, fig.height = 6, fig.cap = "Feature groups defined with a rt window of 20 seconds", eval = FALSE}
plotFeatureGroups(xdata)
grid()
```
...
```{r abundance-correlation-heatmap, fig.cap = "Correlation of features based on feature abundances.", fig.width = 6, fig.height = 16, eval = FALSE}
library(pheatmap)
...
```{r abundance-correlation, eval = FALSE}
...
```{r abundance-correlation-fg040, fig.width = 8, fig.height = 8, fig.cap = "Pairwise correlation plot for all features initially grouped into the feature group FG.040.", eval = FALSE}
...
```{r correlate-eic, message = FALSE, eval = FALSE}
...
```{r correlate-eic-result, eval = FALSE}
table(featureGroups(xdata))
```
...
```{r example-1-eic, fig.width = 8, fig.height = 6, fig.cap = "EICs of features from feature group FG.008.001 in the same sample. Shown are the actual intensities (left) and intensities normalized to 1 (right). Features being part of the same feature group after grouping by EIC similarity are shown in the same color.", eval = FALSE}
...
```{r reset-feature-groups, eval = FALSE}
