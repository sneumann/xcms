...
** Implementation and comparison for =matchedFilter=
...
+ *A*: =do_detectFeatures_matchedFilter=: it's the original code.
+ *B*: =do_detectFeatures_matchedFilter_binYonX_iter=: uses the same sequential
  buffering than the original code, but uses =binYonX= for binning and
  =imputeLinInterpol= for interpolation.
+ *C*: =do_detectFeatures_matchedFilter_no_iter=: contains the original code, but
  avoids sequential buffering, i.e. creates the whole matrix in one go.
+ *D*: =do_detectFeatures_matchedFilter_binYonX_no_iter=: my favorite: uses =binYonX= and
...
*Comparisons*:
+ [ ] *A* /vs/ original: is always OK for =bin=.
+ [ ] *B* /vs/ original: =bin=: OK unless =step= is =0.2=: most likely rounding problem.
+ [ ] *C* /vs/ original: =bin=: OK unless =step= is =0.2=:
+ [ ] *D* /vs/ original: =bin=: OK unless =step= is =0.2=: most likely rounding problem.
+ [ ] *B* /vs/ *C*: =bin=: always OK.
+ [ ] *B* /vs/ *D*: =bin=: always OK.
+ [ ] *C* /vs/ *D*: =bin=: always OK.
