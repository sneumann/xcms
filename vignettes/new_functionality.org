...
specify) and enables saving, reloading and reuse of settings. In addition,
parameter validation can be performed within the parameter object and hence is
no longer required in the analysis function.
...
* Deprecated functions and files				   :noexport:
