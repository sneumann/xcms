...
from all files of an experiment. A new result object for =xcms=-based
preprocessing has also implemented: the =XCMSnExp= object. This is similar to the
old =xcmsSet= object, but by extending the =OnDiskMSnExp= object it provides a
simple and streamlined access to the (raw) data on which the processing was performed.
