...
This document showcases the analysis of two small LC-MS/MS data sets using `r
...
Until the `reconstructChromPeakSpectra` is updated and returns also a `Spectra`
object we have to manually convert our experimental spectrum to the older
`MSpectrum` object.

```{r remove-mspectrum}
ex_spectrum <- reset(ex_spectrum)
sp <- new("Spectrum2", intensity = intensity(ex_spectrum)[[1L]],
          mz = mz(ex_spectrum)[[1L]], centroided = TRUE)
ex_spectrum <- MSpectra(sp)

sp <- new("Spectrum2", intensity = intensity(fenamiphos)[[2L]],
          mz = mz(fenamiphos)[[2L]], centroided = TRUE)
fenamiphos <- MSpectra(sp)
```
...
plot(fenamiphos_swath_spectrum[[1]], fenamiphos[[1]],
...
```{r}
pk_ids <- mcols(fenamiphos_swath_spectrum)$ms2_peak_id[[1]]
pk_ids
```
...
```{r pro-swath-plot, fig.cap = "Mirror plot comparing the reconstructed MS2 spectrum for Prochloraz (upper panel) against the measured spectrum from the DDA data and the Prochloraz spectrum from Metlin.", fig.width = 12, fig.height = 6, eval = FALSE}
par(mfrow = c(1, 2))
