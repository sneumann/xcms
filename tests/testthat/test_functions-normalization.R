...
test_that("applyModelAdjustment works", {
    ...
    ymat <- matrix(rep(y, 5), nrow = 5, byrow = TRUE)
    ymat[2, ] <- ymat[2, ] + 3
    resm <- applyModelAdjustment(ymat, data.frame(ii = inj_idx), lmod = mdl)
    expect_equal(resm[1, ], res)
    expect_true(lm(resm[2, ] ~ inj_idx)$coefficients[2] < 1e-7)

    ## multiple models with a matrix.
    mdls <- rowFitModel(y ~ ii, data = data.frame(ii = inj_idx), y = ymat)
    resm <- applyModelAdjustment(ymat, data.frame(ii = inj_idx), lmod = mdls)
    expect_equal(resm[1, ], res)
